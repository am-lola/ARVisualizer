...
mesh_handle ARVisualizer::AddTriangle(double vertexPositions[3][3], Color color)
{
  ...
  return _renderer->Add3DMesh(MeshFactory::MakeTriangle<Vertex3D>(positions, color));
}
...
mesh_handle ARVisualizer::AddQuad(double center[3], double normal[3], double width, double height, Color color)
{
  ...
  return _renderer->Add3DMesh(MeshFactory::MakeQuad<Vertex3D>(vCenter, vNormal, width, height, color));
}
...
mesh_handle ARVisualizer::AddSphere(double center[3], double radius, Color color)
{
  ...
  return _renderer->Add3DMesh(MeshFactory::MakeIcosphere<Vertex3D>(vCenter, radius, ICOSPHERE_SUBDIV_LEVELS, color));
}
...
mesh_handle ARVisualizer::AddCapsule(double center1[3], double center2[3], double radius, Color color)
{
  ...
  return _renderer->Add3DMesh(MeshFactory::MakeCapsule<Vertex3D>(vCenter1, vCenter2, radius, color, UVSPHERE_RESOLUTION));
}
