...
mesh_handle ARVisualizer::AddTriangle(double vertexPositions[3][3], double color[3])
{
  ...
  glm::vec3 vColor = glm::vec3( color[0], color[1], color[2] );

  return _renderer->Add3DMesh(MeshFactory::MakeTriangle(positions, vColor));
}

mesh_handle ARVisualizer::AddQuad(double* center, double* normal, double width, double height, double* color)
{
